  ...
  <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700">
    <div class="p-6 border-b border-slate-200 dark:border-slate-700">
      <div class="flex items-center gap-3">
        <div class="w-10 h-10 rounded-lg bg-indigo-100 dark:bg-indigo-900/50 grid place-content-center text-xl">✏️</div>
        <div>
          <h1 class="text-xl font-semibold text-slate-900 dark:text-slate-100">Edit User</h1>
          <p class="text-sm text-slate-600 dark:text-slate-400">Update user information and roles</p>
        </div>
      </div>
    </div>
      ...
      <form method="POST" action="{{ route('admin.users.update', $user) }}" class="space-y-6">
        ...
        <div>
          ...
          <input type="password" id="password" name="password" placeholder="Leave blank to keep current password"
                 class="w-full rounded-lg border-slate-300 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-100 focus:border-indigo-500 focus:ring-indigo-500">
          <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">Leave blank to keep the current password</p>
          @error('password')
            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
          @enderror
        </div>
